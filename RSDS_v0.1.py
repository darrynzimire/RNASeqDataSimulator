...
from process_inputFiles import process_countmodel, process_fraglenmodel
import random
...
def GenerateRead(seq, readLen, n, *args):
    ...
    for ag in args:
        ...
        if ag == 'SE':
            ...
            startpos = (random.choices(v, k=n))
            endpos = [i + readLen for i in startpos]

        else:
            nmax = [seqLen - i - 1 for i in readLen]
            v = np.round(np.random.uniform(low=0, high=nmax, size=n))
            startpos = (random.choices(v, k=len(readLen)))

            for p, q in zip(startpos, readLen):
                endpos = (p + q)

    return startpos, seqLen, endpos
...
def sequence_identifier(n):
    header_id = []
    readlen = 101
    for i in range(1, n + 1):
        header = '{}{} {} {}{}'.format('@RSDS_v0.1.', i, i, 'length=', str(readlen))
        header_id.append(header)
...
def main():
    ...
    if args.SE:
        ...
        if countModel == None:
            ...
            counts_s = [round(int(i)) for i in TX_COUNTS]
            counts_s = [1 if x == 0 else x for x in counts_s]
            sample_trans_ids.append(TX_ID)
            COUNTS.append(counts_s)
        
        elif countModel != None and readtot != None:
            
            counts_s = [round(int(i * readtot)) for i in TX_pCOUNT]
        ...
        n = len(reads)
        header = sequence_identifier(n)
        with gzip.open(output + '.fastq.gz', 'wb') as handle:
            for id, read, q in zip(header, reads, quality_strings):
        ...
        seq_ID = sequence_identifier(len(R1))
        print('writing reads to output file' + "\n")
        with gzip.open(output + '_R1.fastq.gz', 'wb') as f1, gzip.open(output + '_R2.fastq.gz', 'wb') as f2:
            for head, reads1, q in zip(seq_ID, R1, quality_strings):
                f1.write('{}\n{}\n+\n{}\n'.format(head, reads1, q).encode())
            for head, reads2, q, in zip(seq_ID, R2, quality_strings):
                f2.write('{}\n{}\n+\n{}\n'.format(head, reads2, q).encode())
